...
class UpdateClientListRepo {
  final ApiQuery apiQuery = ApiQuery();
GetLoginRepo loginRepo=GetLoginRepo();
Session session=Session();
  Future<Response?> updateClientListRepo(ClientModel clientModel) async {
    try {
      String token= await session.tokenExpired();

      LoginModel? loginData =await loginRepo.getUserLoginResponse();
      final response = await apiQuery.postQuery(
          ApiConstants.updateClient,
          // loginData!.tokken,
          token,
          clientModel.toJson());
      return response;
